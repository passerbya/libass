...
void ass_reset_render_context(RenderContext *state, ASS_Style *style)
{
    ...
    state->renderer->state.lineSpacing = style->LineSpacing;
    state->be = 0;
...
static void measure_text(RenderContext *state)
{
    ...
    for (int i = 0; i < text_info->length; i++) {
        ...
        max_line_spacing = FFMAX(max_line_spacing, cur->lineSpacing * state->screen_scale_y);
...
wrap_lines_measure(RenderContext *state, char *unibrks)
{
    ...
    for (i = 0; i < text_info->length; ++i) {
        ...
        if (cur->linebreak) {
            ...
            pen_shift_y += height + state->renderer->settings.line_spacing + max_line_spacing;
            max_line_spacing = -DBL_MAX;
        }
